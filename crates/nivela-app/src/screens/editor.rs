...
fn toolbar(app: &mut NivelaApp, ui: &mut egui::Ui) {
    ...
    ui.horizontal(|ui| {
        tool_button(app, ui, EditorTool::Select, "↖", Text::ToolSelect.tr(locale));
        ...
        .show(ui, |ui| {
            ...
            ui.vertical(|ui| {
                ...
                let Some(asset) = app.selected_asset() else {
                    ui.label(RichText::new(Text::NoClipSelected.tr(locale)).color(theme::TEXT_MUTED));
                    return;
                };
                ...
                prop_row(ui, Text::PropSourceBitrate.tr(locale), &format!("{:.0} Mbps", asset.source_bitrate_mbps));
                ...
                if let Some(l) = &asset.loudness {
                    prop_row(ui, Text::PropLoudness.tr(locale), &format!("{:.1} LUFS", l.integrated_lufs));
                }
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    widgets::tag_accent(ui, &format!("{} → {target:.0} LUFS", Text::NormalizeTo.tr(locale)));
                    widgets::tag_outline(
                        ui,
                        &format!("{} ({:.0} Mbps)", Text::BitrateFromSource.tr(locale), asset.source_bitrate_mbps),
                    );
                });
                ui.add_space(6.0);
                ui.label(
...
fn timeline_panel(app: &NivelaApp, ui: &mut egui::Ui, height: f32) {
    widgets::card_frame().show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ui.label(RichText::new(Text::Timeline.tr(app.locale)).size(11.0).color(theme::TEXT_MUTED));
        });
        ui.separator();
        egui::ScrollArea::vertical().show(ui, |ui| {
