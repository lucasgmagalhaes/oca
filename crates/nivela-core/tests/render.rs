...
use nivela_core::render::{render_export, RenderOutcome};
...
fn renders_and_normalizes_loudness_toward_target() {
    ...
    let output = std::env::temp_dir().join("nivela_core_test_render_ok.mp4");
...
fn cancelling_mid_render_reports_cancelled() {
    let source = fixture("video.mp4");
    let output = std::env::temp_dir().join("nivela_core_test_render_cancelled.mp4");
    let duration_secs = probe_media(&source).unwrap().duration_secs;
    let cancel = AtomicBool::new(false);
    let mut calls = 0;
...
fn rejects_a_missing_source() {
    let output = std::env::temp_dir().join("nivela_core_test_render_missing.mp4");
    let cancel = AtomicBool::new(false);
