...
impl From<oca_avbridge::EncodeError> for RenderError {
    fn from(e: oca_avbridge::EncodeError) -> Self {
        use oca_avbridge::EncodeError as E;
        match e {
...
pub fn render_export(
    ...
    let outcome = oca_avbridge::encode_export(source, output, target_lufs, cancel, |secs| {
    ...
    Ok(match outcome {
        oca_avbridge::EncodeOutcome::Completed => {
            on_progress(100);
            RenderOutcome::Completed
        }
        oca_avbridge::EncodeOutcome::Cancelled => RenderOutcome::Cancelled,
    })
}
