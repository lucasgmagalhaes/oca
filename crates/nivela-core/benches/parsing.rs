...
//! (de)serializing a project. Run with `cargo bench -p nivela-core` (or `make bench`) and
...
use nivela_core::loudness::parse_loudnorm_stderr;
use nivela_core::persistence::{from_json, to_json};
use nivela_core::probe::parse_probe_json;
use nivela_core::timeline::{ClipInstance, Timeline, Track, TrackKind};
use nivela_core::{LoudnessMetrics, MediaAsset, MediaKind, Project, Recency};
...
/// territory) rather than the small mock projects in `nivela_core::sample` — big enough for
