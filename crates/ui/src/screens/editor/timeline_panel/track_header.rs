...
pub(super) fn draw_track_header(
    ...
    // `allocate_ui_with_layout` only advances its parent by the space its contents actually
    // consume. The track controls can be narrower than `track_label_width`, which then starts
    // this row's clip canvas before the ruler's fixed gutter. Reserve the full gutter first,
    // then populate that exact rect without letting the child shrink the parent allocation.
    let header_rect = reserve_track_header(ui, track_label_width, row_height);
...
/// Reserves the fixed track-control gutter shared with the timeline ruler.
fn reserve_track_header(ui: &mut egui::Ui, width: f32, height: f32) -> egui::Rect {
