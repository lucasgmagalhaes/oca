# Changelog — Historical Status Log

Moved from `CLAUDE.md`'s old single-file "Status" section (2026-08-27) — same content,
just relocated so `CLAUDE.md` can stay short. Read this for the *why*/*how* behind a
feature (verification caveats, hard walls, real bugs found along the way). For *what's
done/needed right now*, read `matrix/*.md` instead — this file is a narrative log, not a
checklist. For *what to build next*, read `ROADMAP.md`.
